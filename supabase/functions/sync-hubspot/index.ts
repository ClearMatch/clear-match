...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`Starting HubSpot sync for organization: ${organizationId}`);
    ...
    do {
      try {
        console.log(`Fetching batch ${++batchCount}${after ? ` (after: ${after})` : ''}`);
        ...
        console.log(`Batch ${batchCount}: Processed ${contacts.length} contacts, upserted ${upsertedCount}`);
        ...
        // Continue with next batch on individual batch errors
        break;
      }
    } while (after);

    console.log(`HubSpot sync completed. Total contacts synced: ${totalSynced}`);
